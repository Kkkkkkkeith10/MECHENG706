...
void moving_alone_wall(float target_distance_Sonar, float target_distance_IR, bool using_gyro, bool use_left_side_IRs, bool use_right_side_IRs)
{
  ...
  while(HC_SR04_range() > target_distance_Sonar)
  {
    ...
    // Serial.print(temp_IR_distance_abs);
    // Serial.print(" ");
    // Serial.print(temp_IR_distance_dif);
    // Serial.print(" ");
...
double IR_sensorReadDistance(String sensor)
{
  ...
  {
    distance = 19000 * pow(sensor_value, -0.98);  //distance = 21022 * pow(sensor_value, -0.967)
    if(distance >= 200)
    {
      distance = distance - 5;
    }
  ...
  {
    distance = 142772 * pow(sensor_value, -1.149);
    if(distance >= 250)
    {
