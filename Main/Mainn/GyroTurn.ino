...
void GyroTurn(double target)
{
  ...
  while(currentAngle < (Setpoint - tolerance) || currentAngle > (Setpoint + tolerance))
  {
    if (millis()-previous_millis > 50)
    {
      ...
      left_font_motor.writeMicroseconds(1500 + speed_value);
      left_rear_motor.writeMicroseconds(1500 + speed_value);
      right_rear_motor.writeMicroseconds(1500 + speed_value);
